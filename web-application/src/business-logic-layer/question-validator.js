...
function getErrorsAnswerContent(content) {
    const errors = []
    if (!content) {
        errors.push("Answer is missing.")
    } else if (content.length < ANSWER_CONTENT_MIN_LENGTH) {
        errors.push("Answer is too short.")
    } else if (ANSWER_CONTENT_MAX_LENGTH < content.length) {
...
module.exports = () => {
    ...
    return {
        ...
        getErrorsNewAnswer(answer) {
            const errors = []
            errors.push.apply(errors, getErrorsAnswerContent(answer.content))
            return errors
        },

        getErrorsUpdateAnswer(answer) {
            const errors = []
            errors.push.apply(errors, getErrorsAnswerContent(answer.content))
            return errors
        }
